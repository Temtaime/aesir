...
enum VideoDecoderInitE: VideoDecoderInit_{
	none    = 0x00, ///No flags.
	/**
...
extern(C++) struct Init{
	...
	*/
	void* allocator;
	pragma(mangle, `??0Init@bgfx@@QEAA@XZ`) extern(C++) this(int _);
}
...
extern(C++, "bgfx") struct Attachment{
	...
	extern(D) mixin(joinFnBinds((){
		FnBind[] ret = [
			...
			*/
			{q{void}, q{init}, q{TextureHandle handle, bgfx.impl.Access.Enum access=Access.write, ushort layer=0, ushort numLayers=1, ushort mip=0, ubyte flags=AttachmentE.autoGenMIPs}, ext: `C++`},
		];
		return ret;
	}()));
}
...
mixin(joinFnBinds((){
	FnBind[] ret = [
		...
		*/
		{q{uint}, q{frame}, q{ubyte flags=Frame.none}, ext: `C++, "bgfx"`},
