...
import std.meta, std.range, std.string, std.traits, std.algorithm, perfontain.misc, perfontain.math.matrix;
...
struct BBox
{
	...
	const
	{
		...
		auto collision()(in BBox b)
		{
			...
			return max.zipMap!((a, b) => a < b)(b.min)[].any || min.zipMap!((a, b) => a > b)(b.max)[].any ? F_OUTSIDE : F_INTERSECTS;
		}

		string toString()
		{
			return format(`[ min = %s, max = %s ]`, min, max);
		}
	...
	Vector3 min = MAX_VECTOR, max = -MAX_VECTOR;
private:
