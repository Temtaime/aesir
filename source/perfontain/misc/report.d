...
import std.conv, std.json, std.range, std.string, std.random, std.process,
	std.datetime, std.typecons, std.digest.md, std.algorithm, std.net.curl,
	std.windows.registry, perfontain.opengl, utile.misc, utile.logger;

version (none)  : void configReport(string e, string v, string r, string g)
{
	...
	if (queryBinary(`videoDriver`) != id)
	{
		...
		{
			JSONValue j = [
				`vendor` : e, `version` : v, `renderer` : r, `glsl` : g,
			];
...
{
	JSONValue j = [`error` : e.msg, `file` : e.file, `info` : e.info.toString];
