	...
	auto search(in Vector2s p, in Vector2s t) {
		...
		while(true) {
			...
			if((f & (2 + 1)) == 2+1 && typeOf(x - 1, y + 1) & Cell.Walkable)
				e += addPath(Vector2s(x - 1, y + 1), cast(short)(dist + 4), rp, cast(short)(cost + dc[1] + dc[0] - 6));

			if((f & (2 + 4)) == 2+4 && typeOf(x - 1, y - 1) & Cell.Walkable)
				e += addPath(Vector2s(x - 1, y - 1), cast(short)(dist + 4), rp ,cast(short)(cost + dc[1] + dc[2] - 6));

			if((f & (8 + 4)) == 8+4 && typeOf(x + 1, y - 1) & Cell.Walkable)
				e += addPath(Vector2s(x+1, y - 1), cast(short)(dist + 4), rp, cast(short)(cost + dc[3] + dc[2] - 6));

			if((f & (8 + 1)) == 8+1 && typeOf(x + 1, y + 1) & Cell.Walkable)
				e += addPath(Vector2s(x + 1, y + 1), cast(short)(dist + 4), rp, cast(short)(cost + dc[3] + dc[0] - 6));
...
private:
	...
	void pushPath(uint idx) {
		...
		for(auto i = (h - 1) / 2; h > 0 && _arr[idx].cost < _arr[_heap[i + 1]].cost; i = (h - 1) / 2) {
			_heap[h + 1] = _heap[i + 1];
			h = i;
		}
