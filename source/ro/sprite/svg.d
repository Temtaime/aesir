...
<svg width="%upx" height="%upx" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><defs>`, s.w, s.h);
	...
	foreach(idx, ref f; fs) {
		...
		if(x) ret ~= format(`x="%upx" `, x);
		if(y) ret ~= format(`y="%upx" `, y);

		ret ~= format(`width="%upx" height="%upx" xlink:href="data:image/png;base64,%s"/>`, f.im.w, f.im.h, Base64.encode(f.im.save(`png`).as!ubyte));
	}

	ret ~= `</defs><use xlink:href="#f_0"><animate attributeName="xlink:href" values="`;
	foreach(idx; 0..fs.length) ret ~= format(`%s#f_%x`, idx ? `;` : ``, idx);
