...
{
	this(Layout p, Texture tex, string name, ushort amount)
	{
		super(p, tex);
	...
	override void draw(nk_command_buffer* canvas, nk_rect space)
	{
		if (_amount == 1)
			return;

		auto text = _amount.to!string;
...
protected:
	...
	{
		return format(`%s : %u %s`, _name, _amount, MSG_PCS);
	}
