...
char** stb_readdir_files(const scope char* dir);
char** stb_readdir_files_mask(const scope char* dir, const scope char* wild);
char** stb_readdir_subdirs(const scope char* dir);
char** stb_readdir_subdirs_mask(const scope char* dir, const scope char* wild);
void stb_readdir_free(char** files);
char** stb_readdir_recursive(const scope char* dir, const scope char* filespec);
void stb_delete_directory_recursive(const scope char* dir);
...
uint stb_adler32(uint adler32, const scope void* buffer, uint buflen);
uint stb_crc32_block(uint crc32, const scope void* buffer, uint len);
uint stb_crc32(const scope void* buffer, uint len);

void stb_sha1(ref ubyte[20] output, const scope void* buffer, uint len);
int stb_sha1_file(ref ubyte[20] output, const scope char *file);
